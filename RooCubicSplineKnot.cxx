...
Double_t get(const RooArgList& b, int i, int k) {
  return RooCubicSplineKnot_aux::get(b, i + k);
}

template <typename T>
typename T::const_reference get(const T& t, int i, int j) {
  return t[4 * i + j];
}
template <typename T>
void push_back(T& t, const typename T::value_type& a,
               const typename T::value_type& b, const typename T::value_type& c,
...
void RooCubicSplineKnot::smooth(std::vector<double>& y,
  ...
  assert(size() == n);
  // lambda = 0 : no smoothing ; lambda -> 1: straight line (the ultimate smooth
  // curve)
  if (lambda < 0 || !(lambda < 1)) {
    throw std::string(
        "RooCubicSplineKnot::smooth: smoothing parameter must be in range "
        "[0,1)");
  }
  double mu = 2 * lambda / (3 * (1 - lambda));
  for (int i = 0; i < n - 2; ++i) {
    q[i] = r(i) * (y[i] - y[i + 1]) - r(i + 1) * (y[i + 1] - y[i + 2]);
    uu[i] = p(i + 1) + mu * (sqr(r(i) * dy[i]) + sqr(f(i + 1) * dy[i + 1]) +
                             sqr(r(i + 1) * dy[i + 2]));
    if (i > n - 4) continue;
    vv[i] = h(i + 1) + mu * (f(i + 1) * r(i + 1) * sqr(dy[i + 1]) +
                             f(i + 1) * r(i + 2) * sqr(dy[i + 2]));
    if (i > n - 5) continue;
    ww[i] = mu * r(i + 1) * r(i + 2) * sqr(dy[i + 2]);
  }

  // Solve A b = q  with  A(i,i)=uu(i), A(i+1,i)=A(i,i+1) = vv(i),
  // A(i+2,i),A(i,i+2)=ww(i) start with factorization: A -> L D LT
  vv[0] /= uu[0];
  ww[0] /= uu[0];
  uu[1] -= uu[0] * sqr(vv[0]);
  vv[1] -= uu[0] * vv[0] * ww[0];
  ...
  for (int i = 2; i < n - 4; ++i) {
    ...
    vv[i] -= uu[i - 1] * vv[i - 1] * ww[i - 1];
    vv[i] /= uu[i];
    ww[i] /= uu[i];
  }
  uu[n - 4] -= uu[n - 5] * sqr(vv[n - 5]) + uu[n - 6] * sqr(ww[n - 6]);
  vv[n - 4] -= uu[n - 5] * vv[n - 5] * ww[n - 5];
  vv[n - 4] /= uu[n - 4];
  ...
  // solve for y...
  int i = 0;
  y[i] -= mu * sqr(dy[i]) * (r(i) * q[i]);
  ++i;
  y[i] -= mu * sqr(dy[i]) * (r(i) * q[i] + f(i) * q[i - 1]);
  ++i;
  for (; i < n - 2; ++i)
    y[i] -=
        mu * sqr(dy[i]) * (r(i) * q[i] + f(i) * q[i - 1] + r(i - 1) * q[i - 2]);
  y[i] -= mu * sqr(dy[i]) * (f(i) * q[i - 1] + r(i - 1) * q[i - 2]);
  ++i;
...
void RooCubicSplineKnot::computeCoefficients(std::vector<double>& y,
  ...
  y.front() =
      bc.value[0] - bf * (bc.secondDerivative[0] ? (double(6) / sqr(h(0)))
                                                 : (-double(3) / h(0)));
  y.back() =
      bc.value[1] - bb * (bc.secondDerivative[1] ? (double(6) / sqr(h(n - 2)))
                                                 : (double(3) / h(n - 2)));
  ...
  for (int i = 1; i < n; ++i) {
    double m = double(1) / (mb(i, bc.secondDerivative) -
                            ma(i, bc.secondDerivative) * c.back());
...
void RooCubicSplineKnot::fillPQRS() const {
  assert(_PQRS.empty());
  // P,Q,R,S only depend on the knot vector, so build at construction, and cache
  // them...
...
double RooCubicSplineKnot::evaluate(double x, const RooArgList& b) const {
  ...
  int i =
      index(x);  // location in knot vector: index of last knot less than x...
  assert(-1 <= i && i <= size());
  // we're a 'natural' spline. If beyond first/last knot, Extrapolate using the
  // derivative at the first (last) knot
  if (i == -1) {
    ++i;
    return get(b, i, 0) - d(x, i) * r(i) * (get(b, i, 0) - get(b, i, 1));
  }
  if (i == size()) {
    --i;
    return get(b, i, 2) + d(x, i) * r(i - 1) * (get(b, i, 2) - get(b, i, 1));
  }
  assert(u(i) <= x && x <= u(i + 1));
  return get(b, i, 0) * A(x, i)  // TODO: substitute A,B,C,D 'in situ'
         + get(b, i, 1) * B(x, i) + get(b, i, 2) * C(x, i) +
         get(b, i, 3) * D(x, i);
}
...
double RooCubicSplineKnot::analyticalIntegral(const RooArgList& b) const {
  ...
  if (_IABCD.empty()) {
    // the integrals of A,B,C,D from u(i) to u(i+1) only depend on the knot
    // vector... so we create them 'on demand' and cache the result
    _IABCD.reserve(4 * size());
    for (int j = 0; j < size(); ++j) {
      push_back(
          _IABCD, qua(h(j, j + 1)) / (4 * P(j)),
          -cub(h(j, j + 1)) * (3 * u(j) - 4 * u(j - 2) + u(j + 1)) /
                  (12 * P(j)) -
              sqr(h(j, j + 1)) *
                  (3 * sqr(u(j)) - 2 * u(j - 1) * u(j + 1) + sqr(u(j + 1)) +
                   u(j) * (-4 * u(j - 1) + 2 * u(j + 1) - 4 * u(j + 2)) +
                   6 * u(j - 1) * u(j + 2) - 2 * u(j + 1) * u(j + 2)) /
                  (12 * Q(j)) +
              sqr(h(j, j + 1)) *
                  (3 * sqr(u(j + 1)) + sqr(u(j)) + 2 * u(j) * u(j + 1) -
                   8 * u(j + 1) * u(j + 2) - 4 * u(j) * u(j + 2) +
                   6 * sqr(u(j + 2))) /
                  (12 * R(j)),
          sqr(h(j, j + 1)) *
                  (3 * sqr(u(j)) + sqr(u(j + 1)) + 2 * u(j + 1) * u(j) -
                   8 * u(j) * u(j - 1) - 4 * u(j - 1) * u(j + 1) +
                   6 * sqr(u(j - 1))) /
                  (12 * Q(j)) -
              sqr(h(j, j + 1)) *
                  (3 * sqr(u(j + 1)) + sqr(u(j)) - 4 * u(j - 1) * u(j + 1) +
                   6 * u(j - 1) * u(j + 2) - 4 * u(j + 1) * u(j + 2) -
                   2 * u(j) * (u(j - 1) - u(j + 1) + u(j + 2))) /
                  (12 * R(j)) +
              cub(h(j, j + 1)) * (3 * u(j + 1) - 4 * u(j + 3) + u(j)) /
                  (12 * S(j)),
          qua(h(j, j + 1)) / (4 * S(j)));
    }
  }
  // FIXME: this assumes the integration range goes from first knot to last
  // knot...
...
// location in knot vector: index of last knot less than x... except when beyond
// interval+ last +1 if to right, -1 if to left
int RooCubicSplineKnot::index(double uu) const {
  if (uu > _u.back()) return size();
  std::vector<double>::const_iterator i =
      --std::upper_bound(_u.begin(), _u.end() - 1, uu);
  return std::distance(_u.begin(), i);
}

// S matrix for i-th interval
RooCubicSplineKnot::S_jk RooCubicSplineKnot::S_jk_sum(
    int i, const RooArgList& b) const {
  if (_S_jk.empty()) {
    _S_jk.reserve(size() * 4);
    for (int j = 0; j < size(); ++j) {
      // Thjs 'table' should be compatible wjth the definitions of A,B,C, and
      // D...
      _S_jk.push_back(-RooCubicSplineKnot::S_jk(u(j + 1), u(j + 1), u(j + 1)) /
                      P(j));  // A
      _S_jk.push_back(
          RooCubicSplineKnot::S_jk(u(j - 2), u(j + 1), u(j + 1)) / P(j) +
          RooCubicSplineKnot::S_jk(u(j - 1), u(j + 1), u(j + 2)) / Q(j) +
          RooCubicSplineKnot::S_jk(u(j), u(j + 2), u(j + 2)) / R(j));  // B
      _S_jk.push_back(
          -RooCubicSplineKnot::S_jk(u(j - 1), u(j - 1), u(j + 1)) / Q(j) -
  ...
  using RooCubicSplineKnot_aux::get;
  return get(_S_jk, i, 0) * get(b, i, 0) + get(_S_jk, i, 1) * get(b, i, 1) +
         get(_S_jk, i, 2) * get(b, i, 2) + get(_S_jk, i, 3) * get(b, i, 3);
}
...
RooCubicSplineKnot::S_edge::S_edge(const S_edge& other, double offset)
  ...
  if (offset != 0) {
    std::cout
        << "RooCubicSplitKnot::S_edge: argument \"offset\" is not equal to 0"
        << std::endl;
    assert(offset == 0);
  }
}
...
RooCubicSplineKnot::S_edge RooCubicSplineKnot::S_jk_edge(
    bool left, const RooArgList& b) const {
  using RooCubicSplineKnot_aux::get;
  if (left) {
    // efficiency = return evaluate(u(0),b) -
    // d(x,0)*r(0)*(get(b,0,0)-get(b,0,1));
...
double eI(double lo, double hi, double a, double b, double c, double gamma) {
  ...
  lo *= gamma;
  hi *= gamma;
  double result = -a * b * c * (TMath::Gamma(1, hi) - TMath::Gamma(1, lo)) *
                      TMath::Gamma(1) +
                  (b * c + a * c + a * b) *
                      (TMath::Gamma(2, hi) - TMath::Gamma(2, lo)) *
                      TMath::Gamma(2) -
                  (a + b + c) * (TMath::Gamma(3, hi) - TMath::Gamma(3, lo)) *
                      TMath::Gamma(3) +
...
// return integrals from lo to hi of basis spline . exp(-gamma x), for each
// basis spline...
// FIXME: # of splines != # of knots... so cannot return as TGraphErrors...

// return the chisquared , input hist + gamma, out : coefficients, errors
double RooCubicSplineKnot::expIntegral(const TH1* hist, double gamma,
  ...
  TMatrixD matrix(nsplines, nbins);
  ...
  for (int i = 0; i < nbins; ++i) {
    ...
    // this bit is by construction fully contained between the first and last
    // knot
    for (int j = 0; j < nknots - 1; ++j) {
      double l = std::max(lo, u(j));
      double hh = std::min(hi, u(j + 1));
      if (l >= hh) continue;
      // in thhe knot interval [ u(j),  u(j+1) ], thhe splines j..j+3
      // contribute...
      matrix(j, i) +=
          (-eI(l, hh, u(j + 1), u(j + 1), u(j + 1), gamma) / P(j)) / Norm;
      matrix(j + 1, i) +=
          (eI(l, hh, u(j - 2), u(j + 1), u(j + 1), gamma) / P(j) +
           eI(l, hh, u(j - 1), u(j + 1), u(j + 2), gamma) / Q(j) +
           eI(l, hh, u(j), u(j + 2), u(j + 2), gamma) / R(j)) /
          Norm;
      matrix(j + 2, i) +=
          (-eI(l, hh, u(j - 1), u(j - 1), u(j + 1), gamma) / Q(j) -
           eI(l, hh, u(j - 1), u(j), u(j + 2), gamma) / R(j) -
           eI(l, hh, u(j), u(j), u(j + 3), gamma) / S(j)) /
          Norm;
  ...
  for (int i = 0; i < nsplines; ++i)
    for (int j = 0; j <= i; ++j) {
      ...
      for (int k = 0; k < nbins; ++k)
        DD(i, j) += matrix(i, k) * matrix(j, k) / (DY(k) * DY(k));
  ...
  for (int i = 0; i < nbins; ++i) {
    ...
    double ff = 0;
    for (int j = 0; j < nsplines; ++j) ff += matrix(j, i) * coefficients(j);
    double c = sqr((y - ff) / dy);
    // double lo = hist->GetBinLowEdge(1+i);
    // double hi = lo + hist->GetBinWidth(1+i);
    // std::cout << " bin " << i  << " [  " << lo << " , " << hi << " ]  y = "
    // << y << " dy = " << dy << " f = " << f << " chi^2 = " << c << std::endl;
    chisq += c;
  }
