...
DecRateCoeff::DecRateCoeff(const std::string& name,
...
Int_t DecRateCoeff::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /*rangeName*/) const
{
  // debug
...
Int_t DecRateCoeff::getAnalyticalIntegralWN(RooArgSet& allVars, RooArgSet& analVars, const RooArgSet* normSet, const char* /*rangeName*/) const
{
  // debug
...
std::pair<double, double> DecRateCoeff::calibrate(double eta, double avg_eta, double p0, double p1, double delta_p0, double delta_p1) const
{
  double eta_cal = 0;
  double eta_cal_b = 0;
...
Double_t DecRateCoeff::evaluate(double cp_coeff,
    double cp_coeff_bar,
    ...
    double detection_asym) const
{
  ...
  if(onetagger_) {
    if(coeff_type == kCos){
      if(finalstate == 1){
        ...
        std::cout << "ERROR\t" << "DecRateCoeff::evaluate(...): No valid finalstate!" << std::endl;
        abort();
      }
    }
    if(coeff_type == kSin){
  ...
  if( tag_os == 0 && tag_ss == 0) {
    ...
    std::cout << "ERROR\t" << "DecRateCoeff::evaluate(...): No valid tag combination!" << std::endl;
    abort();
  }
  ...
  if (coeff_type == kSin){
    ...
    std::cout << "ERROR\t" << "DecRateCoeff::evaluate(...): No valid coefficient type!" << std::endl;
    abort();
  }
}
...
bool DecRateCoeff::isFinalstateInRange(const RooCategoryProxy& finalstate, int finalstate_state, const char* rangeName) const
{
...
{
  return dynamic_cast<const RooCategory&>(finalstate.arg()).hasIndex(finalstate_state);
}
