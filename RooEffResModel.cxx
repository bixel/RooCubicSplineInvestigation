...
RooArgList RooEffResModel::CacheElem::containedArgs(Action) {
  ...
  if (_eff)
    return RooArgList(*_int, *_eff, *_x, *_xmin, *_xmax);
  else
...
RooEffResModel::CacheElem::CacheElem(const RooEffResModel& parent,
  ...
  std::unique_ptr<const RooArgSet> effInt(eff.getObservables(iset));

  assert(effInt->getSize() <
         2);  // for now, we only do 1D efficiency histograms...
  if (0 == effInt->getSize()) {
  ...
  std::unique_ptr<std::list<Double_t> > bounds(
      eff.binBoundaries(x, x.getMin(), x.getMax()));
  assert(bounds->size() > 1);
  _bounds.reserve(bounds->size());
  for (std::list<Double_t>::const_iterator lo = bounds->begin(),
                                           hi = ++(bounds->begin());
       hi != bounds->end(); ++hi, ++lo) {
    ...
    if (_bounds.empty()) _bounds.push_back(xmin);
    _bounds.push_back(xmax);
  }
  assert(_bounds.size() > 1);
  // build integral ingredients
  _x = dynamic_cast<RooRealVar*>(
      x.clone((std::string(x.GetName()) + "_" + rn).c_str()));
  assert(_x);
  ...
  // working range
  _xmin = dynamic_cast<RooRealVar*>(
      _x->clone((std::string(_x->GetName()) + "_xmin").c_str()));
  assert(_xmin);
  _xmin->setVal(_bounds.front());
  _xmax = dynamic_cast<RooRealVar*>(
      _x->clone((std::string(_x->GetName()) + "_xmax").c_str()));
  assert(_xmax);
...
RooEffResModel::RooEffResModel(const char* name, const char* title,
    ...
    : RooResolutionModel(name, title, __model.convVar()),
      ...
      _cacheMgr(this) {
  // FIXME: assert that efficiency is a function of convVar, and there are no
  // overlaps...
...
RooEffResModel* RooEffResModel::convolution(RooFormulaVar* inBasis,
  ...
  // Instantiate a clone of this resolution model representing a convolution
  // with given basis function. The owners object name is incorporated in the
  // clones name to avoid multiple convolution objects with the same name in
  // complex PDF structures.
  //
  // Note: The 'inBasis' formula expression must be a RooFormulaVar that encodes
  // the formula in the title of the object and this expression must be an exact
  // match against the implemented basis function strings (see derived class
  // implementation of method basisCode() for those strings
  ...
  if (inBasis->getParameter(0) != x.absArg()) {
    coutE(InputArguments)
        << "RooEffResModel::convolution(" << GetName() << "," << this
  ...
  if (basisCode(inBasis->GetTitle()) == 0) {
    coutE(InputArguments) << "RooEffResModel::convolution(" << GetName() << ","
                          << this << ") basis function '" << inBasis->GetTitle()
  ...
  RooEffResModel* effConv =
      new RooEffResModel(newName.c_str(), newTitle.c_str(), *conv, *eff);
  ...
  if (cacheParamsStr && strlen(cacheParamsStr)) {
    ...
    // const char* ecCacheParamsStr =
    // effConv->getStringAttribute("CACHEPARAMINT"); if (ecCacheParamsStr &&
    // strlen(ecCacheParamsStr)) cout << "bound version has CACHEPARAMINT : " <<
    // ecCacheParamsStr << endl;
    effConv->setStringAttribute("CACHEPARAMINT", cacheParamsStr);
    // cout << "2nd time: bound version has CACHEPARAMINT : " <<
    // effConv->getStringAttribute("CACHEPARAMINT")  << endl; cout << endl <<
    // endl << endl;
  }
...
//_____________________________________________________________________________
void RooEffResModel::initGenerator(Int_t code) { model().initGenerator(code); }
...
RooEffResModel::CacheElem* RooEffResModel::getCache(
  ...
  CacheElem* cache =
      (CacheElem*)_cacheMgr.getObj(iset, &sterileIndex, rangeName);
