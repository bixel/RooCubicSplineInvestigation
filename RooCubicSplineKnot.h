...
class RooCubicSplineKnot {
 public:
  class BoundaryConditions {
   public:
    BoundaryConditions(bool leftSecondDerivative = true,
                       bool rightSecondDerivative = true, double valueLeft = 0,
                       double valueRight = 0) {
      secondDerivative[0] = leftSecondDerivative;
      secondDerivative[1] = rightSecondDerivative;
      value[0] = valueLeft;
      value[1] = valueRight;
    }
  ...
  RooCubicSplineKnot(Iter begin, Iter end) : _u(begin, end) {}
  RooCubicSplineKnot(const std::vector<double>&
                         _knots)  // needed to easily interface with python...
      : _u(_knots) {}
  virtual ~RooCubicSplineKnot() = default;
  ...
  void computeCoefficients(std::vector<double>& y,
                           BoundaryConditions bc = BoundaryConditions()) const;
  void smooth(std::vector<double>& y, const std::vector<double>& dy,
              double lambda) const;
  ...
  class S_jk {
   public:
    ...
    S_jk(const S_jk& other, double offset = 0)
        : t(other.t), d(other.d), s(other.s), o(other.o) {
      if (!offset) return;
    ...
    double operator()(int j, int k) const {
      if (j > k) std::swap(j, k);
      assert(0 <= j && j < 4);
      assert(0 <= k && k < 4 - j);  // note: for 4-j<=k<4 could return 0... but
                                    // better not to invoke with those..
      switch (3 * j + k) {
        case 0:
          return -t;  // (0,0)
        case 1:
  ...
  // as matrix_ij
  double expIntegral(const TH1* hist, double gamma, TVectorD& coefficients,
 ...
 private:
  ...
  double B(double u_, int i) const {
    return sqr(d(u_, i + 1)) * d(u_, i - 2) / P(i) +
           d(u_, i - 1) * d(u_, i + 2) * d(u_, i + 1) / Q(i) +
           d(u_, i) * sqr(d(u_, i + 2)) / R(i);
  }
  double C(double u_, int i) const {
    return -sqr(d(u_, i - 1)) * d(u_, i + 1) / Q(i) -
           d(u_, i) * d(u_, i + 2) * d(u_, i - 1) / R(i) -
           d(u_, i + 3) * sqr(d(u_, i)) / S(i);
  }
  ...
  double ma(int i, bool bc[]) const {  // subdiagonal
    ...
    return (i != size() - 1)
               ? A(u(i), i)
               : (bc[1]) ? double(6) / (h(i, i - 2) * h(i - 1)) : double(0);
  }

  double mc(int i, bool bc[]) const {  // superdiagonal
    return (i != 0) ? C(u(i), i)
                    : bc[0] ? double(6) / (h(2, 0) * h(0)) : double(0);
  }
  ...
  double mb(int i, bool bc[]) const {  // diagonal
    ...
    if (i == 0) {
      ...
      return bc[1]
                 ? -(double(6) / h(i - 1) + double(6) / h(i, i - 2)) / h(i - 1)
                 : -double(3) / h(i - 1);
    }
  }
  ...
  double P(int i) const {
    if (_PQRS.empty()) fillPQRS();
    assert(4 * i < int(_PQRS.size()));
    return _PQRS[4 * i];
  }
  double Q(int i) const {
    if (_PQRS.empty()) fillPQRS();
    assert(4 * i + 1 < int(_PQRS.size()));
    return _PQRS[4 * i + 1];
  }
  double R(int i) const {
    if (_PQRS.empty()) fillPQRS();
    assert(4 * i + 2 < int(_PQRS.size()));
    return _PQRS[4 * i + 2];
  }
  double S(int i) const {
    if (_PQRS.empty()) fillPQRS();
    assert(4 * i + 3 < int(_PQRS.size()));
    return _PQRS[4 * i + 3];
  }
