...
class RooAbsGaussModelEfficiency : public RooAbsReal {
 public:
  ...
  RooAbsGaussModelEfficiency(const char* name, const char* title,
                             const char* unit = "")
      : RooAbsReal(name, title, unit){};
  RooAbsGaussModelEfficiency(const RooAbsGaussModelEfficiency& other,
                             const char* name = 0)
      : RooAbsReal(other, name){};

  ~RooAbsGaussModelEfficiency();

  virtual std::complex<double> productAnalyticalIntegral(
...
class M_n {
 ...
 public:
  ...
  M_n& operator-=(const M_n& other) {
    for (unsigned i = 0; i < MaxOrder; ++i) _m[i] = _m[i] - other._m[i];
    return *this;
  }
