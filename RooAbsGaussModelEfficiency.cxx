...
double L::operator()(unsigned j, unsigned k) const {
  assert(j < 4);
  assert(k < 3);
  switch (k) {
    case 0:
    ...
    case 1:
      switch (j) {
        case 0:
          return 0;
        default:
          return 2 * (*this)(j - 1, 2) / sqrt(TMath::Pi());
      }
    case 2:
      switch (j) {
        case 0:
...
std::complex<double> RooGaussModelAcceptance::K_n::operator()(
  ...
  switch (i) {
    ...
    case 1:
      return 0.5 * _zi * _zi;
    case 2:
      return _zi * (_zi * _zi + 1.0);
    case 3:
      std::complex<double> _zi2 = _zi * _zi;
      return _zi2 * (3. * _zi2 + 3.);
      // TODO: add higher orders!!!! Go up until six... (product of two cubic
      // splines)
  }
  std::cerr << "K_n only implemented upto (and including) 3rd order"
            << std::endl;
  assert(1 == 0);
  return 0;
}
